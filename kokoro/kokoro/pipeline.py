...
class KPipeline:
    ...
    def load_single_voice(self, voice: str):
        ...
        # Always look for voices in the local Kokoro-82M directory
        local_voices_dir = "/home/luokaixuan/a/workspace/python/kokoro-onnx-export/Kokoro-82M/voices"

        if voice.endswith('.pt'):
            f = voice
        else:
            f = os.path.join(local_voices_dir, f"{voice}.pt")
            if not os.path.exists(f):
                raise FileNotFoundError(f"Could not find local voice file: {f}")
