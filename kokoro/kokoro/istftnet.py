...
class AdaINResBlock1(nn.Module):
    def __init__(self, channels, kernel_size=3, dilation=(1, 3, 5), style_dim=64):
        ...
        self.convs1 = nn.ModuleList([
            weight_norm(nn.Conv1d(channels, channels, kernel_size, 1, dilation=d, padding=get_padding(kernel_size, d))) for d in dilation
        ])
        self.convs1.apply(init_weights)
        self.convs2 = nn.ModuleList([
            weight_norm(nn.Conv1d(channels, channels, kernel_size, 1, dilation=1, padding=get_padding(kernel_size, 1))) for _ in range(len(dilation))
        ])
        self.convs2.apply(init_weights)
        self.adain1 = nn.ModuleList([AdaIN1d(style_dim, channels) for _ in range(len(dilation))])
        self.adain2 = nn.ModuleList([AdaIN1d(style_dim, channels) for _ in range(len(dilation))])
...
class Decoder(nn.Module):
    def __init__(self, dim_in, style_dim, dim_out, 
        ...
        super().__init__()
        self.encode = AdainResBlk1d(dim_in + 2, dim_in * 2, style_dim)
        self.decode = nn.ModuleList()
        self.decode.append(AdainResBlk1d(dim_in * 2 + 2 + 64, dim_in * 2, style_dim))
        self.decode.append(AdainResBlk1d(dim_in * 2 + 2 + 64, dim_in * 2, style_dim))
        self.decode.append(AdainResBlk1d(dim_in * 2 + 2 + 64, dim_in * 2, style_dim))
        self.decode.append(AdainResBlk1d(dim_in * 2 + 2 + 64, dim_in, style_dim, upsample=True))
        self.F0_conv = weight_norm(nn.Conv1d(1, 1, kernel_size=3, stride=2, groups=1, padding=1))
        self.N_conv = weight_norm(nn.Conv1d(1, 1, kernel_size=3, stride=2, groups=1, padding=1))
        self.asr_res = nn.Sequential(weight_norm(nn.Conv1d(dim_in, 64, kernel_size=1)))
        self.generator = Generator(style_dim, resblock_kernel_sizes, upsample_rates, 
                                   dim_in, resblock_dilation_sizes, 
